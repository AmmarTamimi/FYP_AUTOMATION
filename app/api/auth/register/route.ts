...
import { sendGroupRegistrationNotification } from "@/app/lib/email";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const deptID = formData.get("deptName") as string;
    ...
    if (!groupUsername || !leaderEmail || !membersJson || !deptID) {
    ...
    console.log("group check complete.");
    ...
    console.log("leader check complete");
    ...
    console.log(
      `✅ ${domains.length} project rows inserted for ProjectID: ${newProjectId}`,
    );
    ...
    admins.forEach(async (admin) => {
      await sendGroupRegistrationNotification(
        admin.username,
        admin.username,
        groupData,
      );
